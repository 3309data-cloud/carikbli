    ...
    try {
      ...
      const [
        ...
        supabase
          ...
          .order('created_at', { ascending: false }), // Urutan terbaru di atas
      ...
      const combined = (fData || []).map(f => ({
        ...f,
        master_kbli: kData?.find(k => k.kode === f.wrong_kode) || { 
          nama: "KBLI Tidak Ditemukan", 
          uraian: "Data master untuk kode ini tidak tersedia di database." 
        }
      }));
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-10 bg-slate-50/50">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden">
            <table className="w-full text-left table-fixed"> {/* Gunakan table-fixed agar kolom lebih stabil */}
              <thead>
              ...
              <tbody className="divide-y divide-slate-100">
                {feedbackList.map((item) => (
                  <tr key={item.id} className="transition-all hover:bg-slate-50/50 align-top">
                    ...
                    {/* Kolom 3: Snapshot Jawaban (Dibuat terlihat semua) */}
                    <td className="px-8 py-6">
